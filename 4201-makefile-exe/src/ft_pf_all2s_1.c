...
/*   Updated: 2023/05/25 01:16:01 by antoda-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_pf_tools_d2s(t_printf *f, unsigned int n, char *str)
{
	...
	i = 0;
	base = ft_radix_char2num((*f).at);
	while (i < (*f).p_num)
	{
		str[i++] = ft_dig2hex((char)(n % base), (*f).at);
		n = n / base;
	}
	return (i);
}
...
int	ft_pf_tools_i2s(char arg_type, unsigned int n, char *str)
{
	...
	i = 0;
	base = ft_radix_char2num(arg_type);
	if (!n)
		str[i++] = '0';
	while (n)
	{
		str[i++] = ft_dig2hex((char)(n % base), arg_type);
		n = n / base;
	}
	return (i);
}
...
/// @param f	struct with the flags (*f).args
/// @param str	string to fill with the result
/// @param arg	float argument to apply function
/// @return		the number of chars of the result
